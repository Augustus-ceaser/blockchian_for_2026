# MedTrust Space Phase 2-A.5 PostgreSQL 数据库设计冻结评审

> **历史版本提示：** 本文是 v1（33 表）冻结基线，已被 [`Phase2-database-design-v2.md`](Phase2-database-design-v2.md) 替代。后续 Catalog、Application、Contract 和 Compute 实现不得继续以本文的 Catalog 四表结构为准。

文档版本：v0.1  
日期：2026-07-22  
状态：数据库逻辑设计冻结候选  
上游文档：`docs/Phase2-domain-model.md`  
...
2. `DataProduct` 属于提供方组织，不属于 Connector；具体 `DataProductVersion` 通过 `product_sources` 映射一个或多个来源 Connector。
3. 不在 `data_products` 中保存 `current_version_id`，避免 Product ↔ Version 循环外键；当前目录版本由 `data_product_publications` 表表达。
4. `Contract` 拆成稳定身份 `contracts` 与不可变内容 `contract_revisions`；签署、标的和 Policy 均绑定具体 revision。
5. `ComputeJob` 固定引用已激活的 `contract_revision_id`，不只引用一个会继续变化的逻辑 Contract。
6. 任务输入通过 `compute_job_inputs` 引用合约标的，不重复保存产品名称和版本号。
7. `Artifact` 与发布授权分离；`artifact_grants` 记录谁可以在何时查看哪个已审制品。
8. `ReviewTask` 保留统一工作项，但使用三个真实外键并以“恰好一个非空”约束绑定产品版本、申请或制品，不采用无外键的通用 `target_type + target_id`。
9. `AuditEvent` 的对象引用保留逻辑多态，不设置业务对象外键，使证据在业务对象停用或未来归档后仍可保留。
10. 所有业务状态由后端显式命令推进；数据库约束负责行内合法性和引用完整性，跨表状态规则由事务内领域服务负责。
...
Connector
            ↓
DataProduct + DataProductVersion + Publication
            ↓
...
| Catalog | `data_products`、`data_product_versions`、`product_sources`、`data_product_publications` |
...
已批准版本内容不可变。目录“当前发布”由 publication 表决定。

| 字段 | 类型 | 键/约束 | 说明 |
|---|---|---|---|
| `id` | uuid | PK | 版本 ID。 |
| `space_id` | uuid | FK → spaces.id，RESTRICT | 冗余租户键，用于查询/RLS；必须与产品相同。 |
| `data_product_id` | uuid | FK → data_products.id，RESTRICT | 逻辑产品。 |
...
| `schema_metadata` | jsonb | NOT NULL | 模态、字段和格式描述。 |
...
| `default_policy_template_ref` | text | NULL | 默认策略模板引用。 |
...
- CHECK：status 为 under_review、approved、retired 时 snapshot_digest 不得为空。
- INDEX：`(space_id, status, created_at DESC)`。
- INDEX：`(data_product_id, status, version_no DESC)`。

不可变策略：

- draft/under_review 可编辑。
- approved/retired 禁止修改内容字段；只允许状态从 approved → retired。
...
| 字段 | 类型 | 键/约束 | 说明 |
|---|---|---|---|
| `data_product_version_id` | uuid | FK → data_product_versions.id，CASCADE 仅限草稿清理 | 产品版本。 |
...
- PK：`(data_product_version_id, connector_id, local_resource_alias)`。
- INDEX：`(connector_id, data_product_version_id)`。

跨表不变量：版本、产品和 Connector 必须属于同一 Space，Connector 必须属于产品提供方或经明确联合授权。

### 6.4 data_product_publications
...
| `id` | uuid | PK | 发布记录 ID。 |
| `space_id` | uuid | FK → spaces.id，RESTRICT | 空间。 |
| `data_product_id` | uuid | FK → data_products.id，RESTRICT | 产品。 |
| `data_product_version_id` | uuid | 与 data_product_id 组成复合 FK → data_product_versions(data_product_id, id)，RESTRICT | 被发布版本，且强制属于同一产品。 |
...
不可变策略：submitted 后用途、产品版本、算法摘要、请求次数和输出类型不可原地改；撤回后新建申请。
...
erDiagram
    ...
    data_product_versions ||--o{ product_sources : sourced_by
...
- approved 后内容、来源、质量、分类分级和 digest 冻结。
- 新增病例、字段或来源必须创建新版本。
- 目录切换版本通过新增/撤回 publication，不修改旧版本内容。
- Application、ContractObject 和 ReviewTask 绑定具体 version ID 与 digest。
...
- product_sources → data_product_versions
...
| product_sources | `(connector_id, data_product_version_id)` | 连接器反查产品。 |
...
5. `data_products`、`data_product_versions`、`product_sources`、`data_product_publications`。
...
- DataProduct 属于组织，Version 映射 Connector。
...
通过。DataProductVersion、ContractRevision 和 Artifact 三层均有稳定 ID、内容摘要和不可覆盖规则。
...
该数据库逻辑设计可以作为 Phase 2-B 建库基线。进入实现时应先完成 Identity、Spaces、Catalog、Applications 的迁移和测试，再实现 Contracts、Compute 与 Audit；不要一次性生成全部 CRUD 后再补状态约束。
